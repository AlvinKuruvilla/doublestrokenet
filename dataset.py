    ...
    for idx, (tstamp, event, key) in enumerate(session):
        ...
        for idx_rel, (tstamp_rel, event_rel, key_rel) in enumerate(session[idx + 1 :]):
            if event_rel == 0 and key_rel == key:
                ...
                # next pressed key
                for idx_next, (tstamp_next, event_next, key_next) in enumerate(
                    session[idx + 1 :]
                ):
                    if event_next == 1:
                        ...
                        for idx_next_rel, (
                            ...
                            event_next_rel,
                            key_next_rel,
                        ) in enumerate(session[idx_next + 1 :]):
...
class BigramDataset(Dataset):
    ...
    def __getitem__(self, index):
        impostor_user_prob = random.choices(
            [0, 1], weights=(1 - self.user_replace_prob, self.user_replace_prob), k=1
        )[0]
        # init target
        ...
        if impostor_user_prob == 1:
            ...
            replace_cnt = max(
                int(curr_replace_prob * len(self.sessions_bigrams_0[index])), 1
            )
            ...
            for repl_idx in to_replace_indices:
                current_bigram = (
                    self.sessions_bigrams_0[index][repl_idx],
                    self.sessions_bigrams_1[index][repl_idx],
                )

                users_with_bigram = list(
                    self.bigr_users[current_bigram] - set([self.users[index]])
                )
        ...
        return (
            self.sessions_bigrams_0[index],
            self.sessions_bigrams_1[index],
            self.sessions_feats[index],
            self.users[index],
            target,
            impostor_user_prob,
        )
...
class BigramPlusDataset(Dataset):
    def __init__(
        ...
        self.user_bigram_count = {
            k: {k1: len(v1) for k1, v1 in v.items()} for k, v in self.users_bigr.items()
        }

    def __getitem__(self, index):
        impostor_user_prob = random.choices(
            [0, 1, 2], weights=self.token_replace_prob, k=1
        )[0]
        # init target
        target = torch.zeros(len(self.sessions_bigrams_0[index]))
        ...
        if impostor_user_prob == 2:
            ...
            current_bigram = (
                self.sessions_bigrams_0[index][random_idx],
            ...
            # replace features from current bigram
            self.sessions_feats[index][random_idx] = random.choice(
                curr_bigram_rnd_feats
            )
