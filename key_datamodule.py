...
from dataset import BigramDatasetVal, BigramPlusDataset
...
class KeyDataModule(pl.LightningDataModule):
    ...
    def split_data(self, user_mapping):
        ...
        for user_file, user_id in user_mapping.items():
            ...
            train_df, test_df = train_test_split(
                user_df, test_size=self.test_size, random_state=42
            )
            # save train and test data

            train_df.to_csv(
                os.path.join(self.train_dir, str(user_id) + ".csv"), index=False
            )
            test_df.to_csv(
                os.path.join(self.test_dir, str(user_id) + ".csv"), index=False
            )
    ...
    def collate_fn(self, batch):
        ...
        b0_padded = torch.nn.utils.rnn.pad_sequence(
            b0, batch_first=True, padding_value=0
        )
        b1_padded = torch.nn.utils.rnn.pad_sequence(
            b1, batch_first=True, padding_value=0
        )
        feat_padded = torch.nn.utils.rnn.pad_sequence(
            feat, batch_first=True, padding_value=0
        )
        target_padded = torch.nn.utils.rnn.pad_sequence(
            target, batch_first=True, padding_value=-1
        )
    ...
    def setup(self, stage=None):
        ...
        if stage == "fit" or stage is None:
            self.train_dataset = BigramPlusDataset(
                self.train_dir,
                self.max_seq_len,
                self.replace_prob,
        ...
        self.val_dataset = BigramDatasetVal(
            self.test_dir, self.max_seq_len, self.val_user_cnt
        )
