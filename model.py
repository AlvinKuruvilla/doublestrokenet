...
class StrokeNet(nn.Module):
    def __init__(
        ...
        self.trf_cross = nn.TransformerEncoder(
    ...
    def forward(self, b0, b1, feat, mask, user, attn_mask=None):
        ...
        if self.causal_att:
            x = self.trf_cross(
                src=x, mask=attn_mask, src_key_padding_mask=mask, is_causal=True
            )
        else:
...
class MultiHeadRot(nn.Module):
    ...
    def forward(self, x, src_mask, src_padding, is_causal):
        ...
        if is_causal:
            dot_prod = F.scaled_dot_product_attention(
                q, k, v, dropout_p=self.active_dropout, is_causal=is_causal
            )
        else:
            dot_prod = F.scaled_dot_product_attention(
                q,
...
class TransformerEncoderRotLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=756, dropout=0.1):
        super().__init__()
...
class TransformerEncoderRot(nn.Module):
    def __init__(self, trf_layer, num_layers):
        super().__init__()
...
class StrokeNetRot(nn.Module):
    ...
    def forward(self, b0, b1, feat, mask, user, attn_mask=None):
        ...
        x = self.input_mlp(x)
        x = self.trf_cross(
            x, src_mask=attn_mask, src_key_padding_mask=mask, is_causal=self.causal_att
        )
