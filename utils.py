...
def split_data(
    user_mapping, train_size, test_size, train_type_dir, raw_data_dir
) -> None:
    """
    ...
    for user_file, user_id in user_mapping.items():
        ...
        train_df, test_df = train_test_split(
            user_df, test_size=test_size, train_size=train_size, random_state=42
        )
        # save train and test data
        train_df.to_csv(os.path.join(train_dir, str(user_id) + ".csv"), index=False)
        test_df.to_csv(os.path.join(test_dir, str(user_id) + ".csv"), index=False)


def split_data_benchmark(
    user_mapping, train_size, test_size, train_type_dir, raw_data_dir
) -> None:
    """
    Split the data into train and test folders.
...
def prepare_data(cfg) -> None:
    ...
    chosen_users = random.sample(
        valid_user_files, cfg.pretrain.data.user_cnt + cfg.finetune.data.user_cnt
    )
    ...
    # split data
    split_data(
        pretrain_user_mapping,
        cfg.pretrain.data.train_size,
        cfg.pretrain.data.test_size,
        cfg.pretrain.data.path,
        cfg.raw_data_path,
    )
    split_data(
        finetune_user_mapping,
        cfg.finetune.data.train_size,
        cfg.finetune.data.test_size,
        cfg.finetune.data.path,
        cfg.raw_data_path,
    )
...
def prepare_data_benchmark(cfg) -> None:
    ...
    # split data
    split_data_benchmark(
        train_user_mapping,
        cfg.pretrain.data.train_size,
        cfg.pretrain.data.test_size,
        cfg.pretrain.data.path_prc,
        cfg.pretrain.data.path,
    )
    split_data_benchmark(
        test_user_mapping,
